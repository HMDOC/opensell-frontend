...
export function AdVisibilityPart(props: {
...
export function AdMapping(props: {
  request: Promise<AxiosResponse<AdBuyerView, any>>;
  children?: any;
}) {
  ...
  const nextOrPrevious = (isNext: boolean) => {
    let newIndex = changePicture(
      isNext,
      currentPicture,
      adBuyerView?.adImages?.length,
    );
  ...
  return (
    <div className="ad-view-position">
      ...
      <div
        style={{ maxWidth: "1400px", overflowY: "scroll", height: "86vh" }}
        ...
        {adBuyerView ? (
          <>
            <div className="user-profil dark-shadow">
              ...
              <Link
                className="user-profil-username"
            ...
            >
              <>
                <FontAwesomeIcon
                  ...
                  onClick={() => setIsPicturePopup(false)}
                  size="2x"
                  icon={faXmark}
                />
                <div className="ad-view-popup-section">
                  <img
                    className="ad-view-popup-image imgFit"
                    src={getAdImageUrl(
                      adBuyerView?.adImages?.[currentPicture]?.path,
                    )}
                    alt="img"
                  />
                  ...
                  <div className="ad-view-popup-controls">
                    <button
                      className="ad-view-popup-image-change-btn"
                      onClick={() => nextOrPrevious(false)}
                    >
                      <img
                        style={{ rotate: "180deg" }}
                        src="/img/prev-img.webp"
                        alt="previous"
                      />
                    </button>

                    <p className="ad-view-popup-image-count">
                      {currentPicture + 1} / {adBuyerView?.adImages?.length}
                    </p>

                    <button
                      className="ad-view-popup-image-change-btn"
                      onClick={() => nextOrPrevious(true)}
                    >
                      <img src="/img/prev-img.webp" alt="next"></img>
                    </button>
                  </div>
                </div>
              </>
            ...
            <AdPricePart
              price={adBuyerView?.adPrice}
              isSold={adBuyerView?.isAdSold}
            />
            ...
            {adBuyerView?.adImages?.length == 0 ? (
              ...
              <div className="ad-view-images">
                <img
                  onClick={() => loadImageFromClick(0)}
                  className="ad-view-first-images ad-view-big-image imgFit"
                  src={getAdImageUrl(adBuyerView?.adImages[0]?.path)}
                />

                <div className="ad-view-images-column">
                  {adBuyerView?.adImages
                    .slice(1, 3)
                    ?.map((value, index) =>
                      value ? (
                        <img
                          key={createRandomKey()}
                          onClick={() => loadImageFromClick(index + 1)}
                          className="ad-view-first-images ad-view-images-column-element imgFit"
                          src={getAdImageUrl(value?.path)}
                          alt="first-image"
                        />
                      ) : (
                        <></>
                  ...
                  {adBuyerView?.adImages.length > 4 ? (
                    ...
                    <p
                      onClick={() => loadImageFromClick(3)}
                      style={{
                        backgroundImage: `url(${getAdImageUrl(adBuyerView?.adImages[3]?.path)})`,
                      }}
                      className="ad-view-images-zoom-in ad-view-first-images ad-view-images-column-element imgFit"
                    >
                      +{adBuyerView?.adImages.length - 4}
                    </p>
                  ) : adBuyerView?.adImages.length == 4 ? (
                    <img
                      onClick={() => loadImageFromClick(3)}
                      src={getAdImageUrl(adBuyerView?.adImages[3]?.path)}
                      className="ad-view-first-images ad-view-images-column-element imgFit"
                      alt="last-image"
                    />
                  ) : (
            ...
            <div className="ad-view-flex-desc-infos">
              <p className="ad-view-description">
                {adBuyerView?.adDescription}
              </p>
              <AdInfosPart
...
const AdView = (): ReactElement => {
  ...
  return <AdMapping request={getAdByLink(link)} />;
};
