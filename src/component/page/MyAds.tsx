...
class DisplayAd extends PureComponent<DisplayAdProps> {
  ...
  public handleDelete(): void {
    deleteAd(this.props.idAd).then((res) => {
      if (res?.data) {
        ...
        <div className="display-post">
          ...
          <div className="display-post-flex-with-img-desc">
            <div className="dislay-post-img-section">
              <img
                className="display-post-img imgFit"
                src={getAdImageUrl(this.props.firstImage)}
                alt="first-image"
              />
            </div>
            ...
            <div className="mt-2 w-100">
              <div className="d-flex justify-content-between">
                <h1 className="my-0">
                  {this.getVisibilityIcon()} {this.props.title}
                </h1>
                <Dropdown>
                  <Dropdown.Toggle
                    size="lg"
                    className="display-dropdown-color"
                  />
                  <Dropdown.Menu variant="light">
                    <DropdownItem
                      as={Link}
                      to={`/u/ad-modification/${this.props.link}`}
                      target="_blank"
                    >
                      Modify
                    </DropdownItem>
                    <DropdownItem
                      as={Button}
                      onClick={() => this.props.seeAdPreview(this.props.idAd)}
                    >
                      Preview
                    </DropdownItem>
                    <DropdownItem
                      bsPrefix="dropdown-item btn-danger"
                      as={Button}
              ...
              <p className="display-description text-break">
                {this.getDescriptionPart()}
              </p>
            </div>
          </div>

          <div className="display-post-options"></div>
        </div>
      </>
    );
  }
...
const MyAds = (props: { idCustomer?: number }) => {
  ...
  const [currentAdPreview, setCurrentAdPreview] =
    useState<Promise<AxiosResponse<AdBuyerView, any>>>();
  const [modalIsOpen, setModalIsOpen] = useState<boolean>(false);

  useEffect(() => {
    if (props.idCustomer && !modalIsOpen) {
      getCustomerAds(props.idCustomer).then((res) => {
        if (res?.data) {
      ...
      {isPreview ? (
        ...
        <div className="back-div">
          ...
          {displayAds.length > 0 ? (
            <>
              <div
                style={{ overflowY: "scroll", height: "87.25vh" }}
                className="ads-view"
              >
